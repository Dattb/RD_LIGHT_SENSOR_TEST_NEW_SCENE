...
//#include "../Rang_Dong/RD_Light_Sensor.h"


extern u16 provision_time;
extern u8 flag_2_sleep;

//extern u16 provision_timeout;
//extern u16 LPN_SCAN_START_TIMEOUT_MS;
...
u8 check_cnt=0;
//u8 check_light=0;
extern  u16 provision_cnt;
void test_cmd_wakeup_lpn()
{

	//RD_light_sensor_tx(0);
...
//	gpio_write(GPIO_PB5,0);                 //LED On
//	gpio_write(GPIO_PB4,1);                 //LED On
	//check_cnt++;
	//if(check_cnt)
	...
	//len += 10;	// test segment;
	mesh_tx_cmd_primary(G_ONOFF_SET_NOACK, (u8 *)&cmd, len, adr_dst, 0);
	//      if((test_onoff%2)) arr[1]= 0x00;
	//      else arr[1]= 0x01;
	//i2c_init();

	#else
	access_cmd_onoff(0xffff, 0, (test_onoff++) & 1, CMD_NO_ACK, 0);
	//mesh_tx_cmd2normal_primary(G_ONOFF_STATUS,arr,4,0x0001,2);
	//mesh_tx_cmd2normal_primary(G_ONOFF_STATUS,&i2c_rx_buff,16,0x0001,0);
	#endif
...
unsigned char check_reset=0;

unsigned char check_cnt_status=0;

u8 reset_cnt=0;
void main_loop ()
{
	if(check_cnt_status==0){
		reset_cnt++;
		RD_Write_reset_cnt(reset_cnt);
		check_cnt_status=1;
	}
//	counter_++;
//	if(counter_>200){
...
//#define     GW_TX_BUFF_DATA_LEN            5
//#define RD_LIGHT_SENSOR_OPCODE             0x01
//#define LIGHT_SENSOR_MODULE_TYPE           0x0002
//u8 gw_tx_buff[GW_TX_BUFF_DATA_LEN] ={0};
